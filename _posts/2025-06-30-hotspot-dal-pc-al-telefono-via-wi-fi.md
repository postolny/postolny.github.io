...
### Обновление драйвера сетевого адаптера на ноутбуке ASUS X550L

Безболезненно обновил драйвер сетевого адаптера [Realtek PCIe GbE Family Controller](https://www.realtek.com/Download/List?cate_id=584){:target="_blank" rel="noopener noreferrer"} до версии 10.76.50.25, выпущенной 14.05.2025 (был установлен драйвер версии 10.45.928.2020). Перед установкой драйвера я создал резервную копию командой в PowerShell, запущенной с правами администратора:

{% highlight bash linenos %}
Copy-Item "C:\Windows\System32\DriverStore\FileRepository\rt640x64.inf_amd64_eecb1d692be08c46" -Destination "C:\DriverBackup\Realtek" -Recurse
{% endhighlight %}

Имя файла rt640x64.inf можно найти в Диспетчере устройств: Свойства → Драйвер → Сведения. Дальше можно перейти в папку `C:\Windows\System32\DriverStore\FileRepository\` и с помощью поиска найти нужную папку с файлами драйвера (в системе может быть несколько версий одного и того же драйвера — для того, чтобы найти активный драйвер, сравните дату и версию драйвера, указанные в его свойствах (Свойства → Драйвер) с датой и версией в файле rt640x64.inf).

Ели вы хотите сделать бекап всех имеющихся в системе драйверов, то можете воспользоваться командой:
