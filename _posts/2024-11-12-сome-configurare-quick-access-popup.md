...
- **Гибернация (CMD)** (если включениа) `/c shutdown /h` (если Гибернация выключена, вы можете включить её командой PowerShell `powercfg /hibernate on` (или короче `powercfg -h on`), после чего команду `shutdown /h` можно будет запустить как в Командной строке, так и в PowerShell).
