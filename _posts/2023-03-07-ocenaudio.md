...
На [сайте](https://www.macaulaylibrary.org/resources/audio-editing-tutorials/prepare-sound-files-in-ocenaudio/) медиатеки Маколея[^1] я нашёл информацию, что пиковый уровень записей, хранящихся в этой медиатеке, составляет -3dB и, насколько я понимаю, является неким стандартом.

### Запись

Я, конечно, не звукорежжисёр и не специалист по звуку, но кое-чему пришлось научиться и мне. В основном я записываю мою игру на фортепиано – это очень полезно – слушать себя со стороны. Микрофон у меня USB – один из самых недорогих, с кардиоидной направленностью (возможно мне стоило приобрести всенаправленный микрофон). Конечно же это монофонический микрофон и в аудиоредакторе я тоже выбираю моно. В свойствах микрофона я выбираю стандартные значения разрядности и частоты дискретизации: 24 бит, 44100 Гц или 24 бит, 48000 Гц:

![свойства микрофона]({{ site.baseurl }}/img/ocenaudio/microphone-properties.png)

Звуковая карта позволяет также выбрать частоту дискретизации 192000 Гц, и если вы хотите порадовать летучих мышей или дельфинов и записать для них что-нибудь в ультразвуковом диапазоне, то можете выбрать такую частоту дискретизации. Для человека же это не имеет никакого смысла.

При первом подключении USB микрофона в систему устанавливается драйвер и микрофон становится устройством записи по умолчанию. Если аудиоредактор "не видит" микрофон, нужно выбрать его в "Настройках звука" программы (в разделе "Устройства/параметры записи" выбран мой микрофон):
...
Для того, чтобы начать запись можно непосредственно кликнуть на значок записи в виде красного круга – откроется окно "Новый звук", где я указываю разрешение 24 бит, частоту дискретизации не больше 48000 Гц (48 кГц) и в разделе "Каналы" выбираю "Моно":
...
Когда в разделе "Каналы" следует выбирать "Стерео" (т.е. два канала)? В том случае, если вы записываете двумя микрофонами или стереомикрофоном. В каких это бывает случаях? Когда вы записываете оркестр или большой музыкальный инструмент (например, пианино или рояль) или может быть птичек в лесу. Но если вы записываете голос, то микрофон конечно же должен быть монофоническим. В аудиоредаторе, конечно можно записать монофонический сигнал на два канала, указав "Стерео" в разделе "Каналы", но такая запись не превратится в стереофоническую, хоть и будет двухканальной.

Поскольку микрофон имеет невысокий уровень записи звука, я обычно после записи или несколько увеличиваю амплитуду звука вручную или применяю "Нормализацию". Шум я не убираю, хотя иногда это и стоило бы сделать.
