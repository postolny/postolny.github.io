...
2. Установить MuseScore 2 ([MuseScore-2.3.0.msi](https://ftp.osuosl.org/pub/musescore/releases/MuseScore-2.3/));
...
5. "Файл" → "Создать сборник..." → в открывшемся окне "Создание сборника произведений" добавить партитуры (кнопка "Добавить Партитуру...") и выбрать все сохранённые файлы в формате MuseScore .mscz → в поле "Название сборника ввести название сборника → нажать на кнопку "Объединить" и сохранить сборник в формате MuseScore .mscz;
6. Теперь можно открыть созданный альбом в формате MuseScore .mscz в MuseScore 2 или MuseScore 3 и экпортировать в нужный формат или распечатать.
...
![invenzione]({{ site.baseurl }}/img/musescore/album1.png){: .lightbox}
...
![invenzione]({{ site.baseurl }}/img/musescore/album2.png){: .lightbox}
...
![invenzione]({{ site.baseurl }}/img/musescore/album3.png){: .lightbox}

Затем нужно добавить две горизонтальные рамки: одна рамка будет служить для указания названия и автора произведения; вторая – для расшифровки мордента (как вставлять рамки читайте в разделе <span class="toc"><a href="#cornici">Рамки</a></span>). Отступ сверху нижней рамки нужно будет уменьшить:

![invenzione]({{ site.baseurl }}/img/musescore/album4.png){: .lightbox}

Теперь нужно будет перейти на вкладку с партитурой фуги, выделить весь такт (для этого нужно последовательно кликнуть на оба нотоносца, удерживая клавишу <kbd>Shift</kbd>) и, перейдя на вкладку с партитурой инвенции, вставить скопированный такт в подготовленный такт. Затем последовательно выделяя и копируя текст названия и автора произведения, расшифровку мордента (выделяя и нажимая <kbd>Ctrl</kbd> + <kbd>S</kbd>), нужно будет вставить их в соответствующие рамки (<kbd>Ctrl</kbd> + <kbd>V</kbd>). Высоту рамки с расшифровкой мордента можно уменьшить:

![invenzione]({{ site.baseurl }}/img/musescore/album5.png){: .lightbox}
